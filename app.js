...
/** Normalize any path into a safe absolute URL for iOS Safari */
function toAbsUrl(path){
  const raw = String(path ?? "").trim();
  if(!raw) throw new Error("Story file path is empty.");
  // Convert to absolute URL anchored to current page
  return new URL(raw, window.location.href).toString();
}

async function fetchJson(urlOrPath){
  const abs = toAbsUrl(urlOrPath);
...
async function loadStoryById(storyId){
  ...
  if(!filePath) throw new Error(`Story "${storyId}" is missing "file" in stories.json`);

  const story = await fetchJson(filePath);
